var wubi = (function (){
    ...
    Wubi.fn = Wubi.prototype = {
        init: function (ops){
            this.options = extend(options, ops);
        },
        initialize: function(ops){
            this.init(ops);
        	this.wubiArray=new Array(
        ...
        getCamelChars: function(str){
            ...
            for (var i=0, len = str.length; i < len; i++){
                var ch = str.substr(i,1), ascii = ch.charCodeAt();
                if(ascii >= 1 && ascii <= 254){
                    result += ch;
                }else{
                    name = this._capitalize(ch);
                    if(name != ""){
                        result += name;
                    }
                }
        ...
        _capitalize: function(str){
            for (var key in this.wubiArray){
                if(-1 != this.wubiArray[key].indexOf(str)){
                    return this.wubiArray[key].substr(0,1); break;
                }
            }
            return false;
        }
